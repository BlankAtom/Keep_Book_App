...
class MoneyInputFilter : InputFilter {
    ...
    override fun filter(
        ...
        dend: Int       //d start+将会被替换的字符串长度
    ): CharSequence {//方法返回的值将会替换掉dest字符串中d start位置到d end位置之间字符，返回source表示不做任何处理，返回空字符串""表示不输入任何字符
        Log.d("MoneyInputFilter", "[source:$source][start:$start][end:$end][dest:$dest][d-start:$dstart][d-end:$dend]")

        val startStr = dest.subSequence(0, dstart)
        val endStr = dest.subSequence(dend, dest.length)
        val target = startStr.toString() + source + endStr//字符串变化后的结果
